...
# ── Config ─────────────────────────────────────────────────────────
INPUT_FILE    = os.path.join(DATA_DIR, "clusters3.csv")
RANDOM_SEED   = 42
RR_RESTARTS   = 50        # how many random restarts for Algorithm 3
# 50 was chosen as a balance between solution quality and runtime.
# Each restart tries a new random order for 2-opt; more restarts give 
# diminishing returns, but too few reduce chances of good solutions.
# For typical DPD van cluster sizes (25–55 stops), 50 restarts gives
# consistently strong results within a reasonable total runtime (~seconds per van).
...
save_routes_csv(os.path.join(DATA_DIR, "routes3_nn.csv"),    "nn")
save_routes_csv(os.path.join(DATA_DIR, "routes3_2opt.csv"),  "2opt")
save_routes_csv(os.path.join(DATA_DIR, "routes3_rr2opt.csv"),"rr")
...
with open("routing_summary3.txt", "w", encoding="utf-8") as f:
    f.write(summary_text)

print("\n  Saved: routing_summary3.txt")
print("\n" + summary_text)